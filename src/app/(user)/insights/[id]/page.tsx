...
// ─────────────────────────────────────────────────────────────────────────────
// TradeInsight Daily — Detailed Insight Reading Page
//
// Architecture:
//   • Centered max-w-3xl reading column (Substack/Medium pattern)
//   • 4-case access control gate (logged-out / free-fundamental /
//     free-ICT-today / pro) driven by two mock booleans
//   • 30% SEO snippet via character-slice + CSS mask-image gradient fade
//   • Glassmorphism paywall cards (login / upgrade) anchored below the snippet
//   • Reading-progress bar (sky-blue, fixed top)
//   • AI Language Switcher dropdown (12 languages, UI-only)
//   • Sticky bottom action bar (Bookmark · Share · Translate)
//   • Full Precision Minimalism design system (zinc-950, Outfit, semantic colour tokens)
// ─────────────────────────────────────────────────────────────────────────────
...
// Replace these with real session data:
//   const session = await getSession(); const isLoggedIn = !!session;
//   const hasProAccess = session?.user?.role === "pro";

const IS_LOGGED_IN   = false;   // ← toggle for testing
const HAS_PRO_ACCESS = false;   // ← toggle for testing
...
// ─── SHARED MOCK DATA ─────────────────────────────────────────────────────────
// Source of truth — mirrors dashboard/page.tsx INSIGHTS array exactly.
// Wire to DB: const insight = await getPost(params.id);
...
function resolveAccess(insight: Insight | null): AccessCase {
  ...
  // Historical ICT biases are always free (time-delayed unlock)
  if (insight.isHistorical) return "full";
  // Non-pro content is always free
  if (!insight.isProOnly) return "full";
  // isProOnly = true && not historical
  if (!IS_LOGGED_IN) return "login-gate";
  return "pro-gate";
}

// Returns the first ~30% of the detail text, split at a sentence boundary
function getSnippet(text: string): string {
  const targetLen = Math.floor(text.length * 0.3);
  // Find the last sentence-ending punctuation at or before targetLen
  const slice = text.slice(0, targetLen + 60); // small lookahead
      ...
      {open && (
        <div
          className="
            ...
            rounded-xl border border-zinc-800/80 bg-zinc-950/95 backdrop-blur-xl
  ...
  return (
    <div
      className="
        ...
        border border-zinc-700/50 bg-zinc-900/80 backdrop-blur-xl
    ...
    >
      ...
      <div className="relative">
        ...
        <p className="mb-5 text-[12px] font-light leading-relaxed text-zinc-500">
          Create a free account to read the full analysis. Fundamental insights
          are always free — no credit card required.
        </p>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:gap-3">
          <Link
            ...
            className="
              ...
              rounded-xl border border-sky-500/40 bg-sky-500/15
              px-4 py-2.5 text-[12px] font-semibold text-sky-400
              transition-all duration-150
              hover:bg-sky-500/25 hover:border-sky-500/60
          ...
          <Link
            ...
            className="
              flex flex-1 items-center justify-center gap-2
              rounded-xl border border-zinc-700/60 bg-zinc-800/60
              px-4 py-2.5 text-[12px] font-semibold text-zinc-300
              transition-all duration-150
...
function ProGateCard({ asset }: { asset: string }) {
  return (
    <div
      className="
        relative overflow-hidden rounded-2xl
        border border-amber-500/20 bg-zinc-900/80 backdrop-blur-xl
        px-6 py-6 sm:px-8 sm:py-7
        shadow-2xl shadow-black/40
      "
    >
      ...
      <div className="relative">
        ...
        <h3 className="mb-1 text-[15px] font-bold text-white">
          Unlock Today's ICT Bias for {asset}
        </h3>
        <p className="mb-5 text-[12px] font-light leading-relaxed text-zinc-500">
          Today's ICT bias — including Order Blocks, FVGs, liquidity targets, and
          exact entry/exit levels — is reserved for Pro subscribers. Upgrade to
          access all live daily biases before the market opens.
        </p>
        ...
        <Link
          ...
          className="
            ...
            rounded-xl border border-amber-500/35 bg-amber-500/12
            px-5 py-3 text-[12.5px] font-semibold text-amber-400
            transition-all duration-150
            hover:bg-amber-500/22 hover:border-amber-500/55
            hover:shadow-[0_0_24px_rgba(245,158,11,0.18)]
          "
        >
          <Sparkles size={12} strokeWidth={2} />
          Upgrade to Pro
          <span className="ml-auto text-[10px] font-normal text-amber-500/60">
            Unlock instantly →
          </span>
        </Link>
...
function ArticleBody({
  ...
  // Render text with paragraph breaks preserved
    ...
    <div className="relative">
      ...
      <div
        ...
        style={
          !showFull
            ? {
                // Premium gradient mask: text fades to transparent before the paywall
                WebkitMaskImage:
      ...
      >
        {paragraphs.map((para, i) => {
          // Detect bullet-style lines (starting with •)
          if (para.startsWith("•")) {
            return (
          ...
          return (
            <p
              key={i}
              className="mb-4 text-[13.5px] font-light leading-[1.85] text-zinc-400"
            >
              {para}
            </p>
          );
      ...
      {/* Paywall card — positioned so it overlaps the fade */}
      {!showFull && (
        <div className="mt-[-2rem]">
          {accessCase === "login-gate" ? (
            <LoginGateCard />
          ) : (
            <ProGateCard asset={asset} />
          )}
        </div>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onBookmark}
          title={isBookmarked ? "Remove bookmark" : "Save insight"}
          className={`
        ...
        <button
          ...
          className="
            ...
            px-3.5 py-2 text-[11px] font-semibold text-zinc-500
            transition-all duration-150
        ...
        <button
          ...
          className="
            ...
            px-3.5 py-2 text-[11px] font-semibold text-zinc-500
            transition-all duration-150
            hover:bg-sky-500/10 hover:text-sky-400 hover:border-sky-500/20
          "
        >
...
function NotFound() {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-950 px-4 text-center">
      ...
      <Link
        href="/dashboard"
        className="inline-flex items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-900/60 px-4 py-2 text-sm font-semibold text-zinc-300 transition-all hover:border-zinc-700 hover:text-white"
      >
        <ArrowLeft size={14} />
        Back to Dashboard
      </Link>
...
export default function InsightDetailPage() {
  ...
  const insightId  = Number(params?.id);

  const [isBookmarked, setIsBookmarked] = useState(false);
  const [showToast,    setShowToast]    = useState(false);

  // Find insight from mock data (will be DB query in production)
  ...
  // Determine if this is a "Past Pro" historical insight
  ...
  return (
    <>
      ...
      {/*
       * Root shell: full-screen dark canvas.
       * pb-28 gives the sticky action bar room to breathe.
       */}
      <div className="min-h-screen w-full overflow-x-hidden bg-zinc-950 text-white antialiased selection:bg-sky-500/30 selection:text-sky-200">
        ...
        >
          ...
          <Link
            ...
            className="
              ...
              rounded-lg border border-transparent px-2.5 py-1.5
          ...
          {/* Centre: brand wordmark (optional — hidden on very small screens) */}
          ...
          <button
            onClick={handleShare}
            className="
              flex items-center gap-1.5 rounded-lg border border-transparent px-2.5 py-1.5
              text-[11.5px] font-semibold text-zinc-500
              transition-all duration-150
              hover:border-zinc-800 hover:bg-zinc-900/60 hover:text-zinc-200
            "
        ...
        <main className="mx-auto w-full max-w-3xl px-4 pb-32 sm:px-6 lg:px-8">
          ...
          <header
            className={`
              relative mt-8 mb-8 overflow-hidden rounded-2xl
              border border-zinc-800/70 bg-zinc-900/40
              bg-gradient-to-br ${dir.headerAccent} to-transparent
          ...
          >
            ...
            <div className="relative">
              {/* Row 1: Category + Past Pro badge + Direction */}
              ...
              <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-zinc-800/60 pt-4">
                ...
                <span className="flex items-center gap-1.5 text-[10.5px] text-zinc-600">
                  ...
                  {insight.readMin} min read
                </span>

                {/* Confidence */}
                <div className="ml-auto flex min-w-[120px] items-center gap-2">
                  <span className="text-[9.5px] font-semibold uppercase tracking-widest text-zinc-700">
                    Confidence
                  </span>
          ...
          {isPastPro && (
            <div className="mb-6 flex items-start gap-3 rounded-xl border border-violet-500/15 bg-violet-500/[0.05] px-4 py-3.5">
              ...
              <div>
                <p className="text-[11.5px] font-semibold text-violet-300">
                  Verified Historical Bias
                </p>
          ...
          <div className="mb-7 flex items-center justify-between gap-3 rounded-xl border border-zinc-800/60 bg-zinc-900/30 px-3.5 py-2.5">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded border border-zinc-800 bg-zinc-900 px-1.5 py-0.5 text-[8.5px] font-bold uppercase tracking-wider text-zinc-600">
                Beta
              </span>
            </div>
            <LanguageSwitcher />
          ...
          {/* ── FULL ACCESS: Pro or Free-eligible ─────────────────────────── */}
          {accessCase === "full" && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10.5px] font-light text-zinc-600">
                    {HAS_PRO_ACCESS
                      ? `All ${insight.readMin}-min analysis unlocked. Published before market open.`
          ...
          <div className="mt-10 flex items-center justify-center">
            <Link
              ...
              className="
                ...
                px-4 py-2.5 text-[11.5px] font-semibold text-zinc-500
                transition-all duration-150
                hover:border-zinc-700 hover:bg-zinc-900/70 hover:text-zinc-200
              "
            >
