...
// src/app/(user)/calendar/CalendarPageClient.tsx
// ─────────────────────────────────────────────────────────────────────────────
// TradeInsight Daily — Smart Trade Setup Engine  v3  "Real-Auth Session Wired"
//
// v3 changes vs v2:
//   [AUTH] Removed hardcoded `HAS_PRO_ACCESS = false` and `IS_LOGGED_IN = false`
//          module-level constants.  The component now receives `hasProAccess`
//          as a prop from the server shell (calendar/page.tsx), which derives
//          the value from the real Auth.js JWT session.
//
//   [KEEP] All v2 layout, animations, and Deep Data Panel behaviour unchanged.
// ─────────────────────────────────────────────────────────────────────────────
...
const ECONOMIC_EVENTS: EconomicEvent[] = [
  ...
  },
  {
    ...
    aiStatus: "processing",
    explanation:
      "Claimant Count Change measures the monthly change in the number of people claiming unemployment benefits in the UK. Lower claims = tighter labour market = wage pressure = potential BoE rate hikes = GBP bullish. Higher claims suggest economic slack, which typically weakens Sterling.",
    history: [
  ...
  },
  {
    ...
    aiStatus: "ready",
    explanation:
      "The ECB Press Conference follows the rate decision and is where markets really react. President Lagarde's tone — hawkish vs dovish — can move Euro pairs by 80–150 pips in minutes.",
    history: [
  ...
  },
  {
    ...
    aiStatus: "processing",
    explanation:
      "Initial Jobless Claims is a weekly snapshot of how many Americans filed for unemployment benefits for the first time. Fewer claims → labour market is tight → Fed stays hawkish → Dollar strengthens.",
    history: [
  ...
  },
  {
    ...
    aiStatus: "processing",
    explanation:
      "The German Ifo Business Climate Index surveys around 9,000 German firms on their current business situation and 6-month outlook. A surprise to the upside lifts the Euro.",
    history: [
  ...
  },
  {
    ...
    aiStatus: "processing",
    explanation:
      "Nonfarm Payrolls (NFP) is the single most market-moving economic event in the world. A strong beat means the Fed has room to keep rates elevated — Dollar rips higher, Gold tumbles.",
    history: [
...
const MOCK_AI_SETUPS: Record<number, {
  ...
  4: {
    ...
    detail: "Lagarde's press conferences are notorious for re-pricing the Euro regardless of the rate decision itself.",
    pairs: [
    ...
    <div className="relative overflow-hidden rounded-2xl border border-rose-500/25 bg-gradient-to-br from-rose-500/[0.08] via-zinc-900/50 to-zinc-900/40 backdrop-blur-sm shadow-2xl shadow-rose-500/8">
      ...
      <div className="relative flex items-center justify-between gap-2 border-b border-zinc-800/50 px-4 py-2.5 sm:px-5">
        ...
        <span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${ai.colour} ${ai.border} ${ai.bg}`}>
          <Activity size={8} strokeWidth={2.5} />{ai.label}
        </span>
      </div>
      <div className="relative px-4 py-4 sm:px-5 sm:py-5">
        ...
        <h2 className="mb-0.5 text-xl font-extrabold leading-tight tracking-tight text-white sm:text-2xl">{event.title}</h2>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-1.5">
          ...
          {event.affectedAssets.map((a) => (
            <span key={a} className="rounded-md border border-zinc-700/50 bg-zinc-800/50 px-2 py-0.5 text-[9.5px] font-semibold text-zinc-400">{a}</span>
          ))}
        </div>
      ...
      <div className="relative flex items-center justify-between gap-3 border-t border-zinc-800/50 px-4 py-3 sm:px-5">
        ...
        <button className="flex items-center gap-1.5 rounded-lg border border-zinc-700/50 bg-zinc-800/50 px-3 py-1.5 text-[10px] font-semibold text-zinc-400 transition-all hover:border-zinc-600 hover:text-zinc-200">
          <Bell size={9} strokeWidth={2} />Set Alert<ChevronRight size={9} />
        </button>
      </div>
    </div>
  );
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <div className="mb-1.5 flex items-center gap-1.5">
                  <span className={`rounded border px-1 py-0.5 text-[8px] font-extrabold uppercase ${cur.bg} ${cur.text} ${cur.border}`}>{ev.currency}</span>
...
function FilterBar({ activeDay, onDay, highOnly, onHighOnly }: {
      ...
      <div className="flex items-center gap-2 overflow-x-auto scrollbar-none [&::-webkit-scrollbar]:hidden">
        <div className="flex shrink-0 items-center gap-1.5">
          {DAY_TABS.map((day) => (
            ...
            >
              <Calendar size={9} strokeWidth={activeDay === day ? 2.2 : 1.75} />{day}
            </button>
          ))}
        </div>
        ...
        <div className="ml-auto hidden shrink-0 items-center gap-4 sm:flex">
          {(["High", "Medium", "Low"] as Impact[]).map((lvl) => (
            <span key={lvl} className="flex items-center gap-1.5 text-[9.5px] text-zinc-600">
              <span className={`h-1.5 w-1.5 rounded-full ${IMPACT_CFG[lvl].dot}`} />{lvl}
            </span>
          ))}
        </div>
      </div>
...
// ─── DEEP DATA PANEL — Tab 1: Explanation ─────────────────────────────────────
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto rounded-md border border-emerald-500/25 bg-emerald-500/10 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider text-emerald-400">Free</span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-zinc-800/50 pt-3">
        ...
        {event.affectedAssets.map((a) => (
          <span key={a} className="rounded-md border border-zinc-700/50 bg-zinc-800/40 px-2 py-0.5 text-[10px] font-semibold text-zinc-300">{a}</span>
        ))}
      </div>
    </div>
  );
    ...
    <div className="relative overflow-hidden rounded-xl border border-amber-500/20 bg-zinc-900/90 px-5 py-5 backdrop-blur-md shadow-2xl shadow-black/50">
      ...
      <div className="relative">
        ...
        <div className="mb-4 grid grid-cols-2 gap-y-1.5 gap-x-4">
          {["Full 12-month history","Actual vs Forecast delta","Pip move per release","Volatility heatmap","Beat/miss streaks","Seasonal patterns"].map((f) => (
        ...
        <Link href="/pricing" className="flex w-full items-center justify-center gap-2 rounded-xl border border-amber-500/35 bg-amber-500/12 py-2.5 text-[11.5px] font-semibold text-amber-400 transition-all hover:bg-amber-500/20 hover:border-amber-500/55 hover:shadow-[0_0_20px_rgba(245,158,11,0.15)]">
          <Sparkles size={11} />Unlock Full History & Volatility Data<ArrowRight size={10} className="ml-auto" />
        </Link>
      </div>
    </div>
  );
...
function HistoryTab({ event, hasProAccess }: { event: EconomicEvent; hasProAccess: boolean }) {
  const visibleRows = hasProAccess ? event.history : event.history.slice(0, FREE_ROWS);
  const hasGate     = !hasProAccess && event.history.length > FREE_ROWS;
    ...
    <div>
        ...
        {visibleRows.map((row, i) => {
          ...
          return (
            <div key={i} className={`grid grid-cols-4 items-center gap-2 rounded-lg px-2 py-2.5 ${i % 2 === 0 ? "bg-zinc-800/20" : ""}`}>
              ...
              <span className={`text-[11px] font-semibold tabular-nums ${getActualColour(row.actual, row.forecast)}`}>{row.actual}</span>
              <span className="text-[11px] text-zinc-500 tabular-nums">{row.forecast}</span>
              <span className={`inline-flex w-fit items-center rounded-md border px-1.5 py-0.5 text-[10px] font-bold tabular-nums ${pc.text} ${pc.bg} ${pc.border}`}>{row.pipMove}</span>
            </div>
          );
        })}
      </div>
      {hasGate && (
        <div className="relative mt-1">
          ...
          <div className="relative mt-[-0.25rem]"><HistoryGateCard /></div>
        </div>
      )}
    </div>
  );
...
// ─── DEEP DATA PANEL — Tab 3: AI Smart Insight ────────────────────────────────
        ...
        {event.aiStatus === "processing" ? (
          <div className="mb-4 flex items-start gap-2 rounded-lg border border-zinc-800/60 bg-zinc-900/40 px-3 py-2.5">
            ...
            <p className="text-[11.5px] font-light leading-relaxed text-zinc-600">
              AI analysis hasn't started yet — it begins 1 hour before release to incorporate the freshest positioning data.
            </p>
          </div>
        ) : (
          <div className="mb-4 rounded-lg border border-amber-500/15 bg-amber-500/[0.05] px-3 py-2.5">
            <p className="text-[11.5px] font-light leading-relaxed text-zinc-400">
              {event.aiStatus === "ready"
                ? `AI Bias is ready for ${event.title}. Upgrade to unlock the full directional bias, institutional order flow read, and trade setup.`
                : `Live AI Signal is active for ${event.title}. Upgrade to view the full setup.`
              }
            </p>
          </div>
        )}
        <div className="mb-4 grid grid-cols-2 gap-y-1.5 gap-x-4">
          {["Directional bias (Bull/Bear)","Institutional order flow","Entry zone & SL/TP levels","Liquidity target mapping","Pre/post release playbook","Volatility range estimate"].map((f) => (
            <span key={f} className="flex items-center gap-1.5 text-[10px] text-zinc-600">
              <Brain size={8} className="shrink-0 text-violet-600" strokeWidth={2} />{f}
            </span>
          ))}
        </div>
        <Link href="/pricing" className="flex w-full items-center justify-center gap-2 rounded-xl border border-amber-500/35 bg-amber-500/12 py-2.5 text-[11.5px] font-semibold text-amber-400 transition-all hover:bg-amber-500/20 hover:shadow-[0_0_18px_rgba(245,158,11,0.14)]">
          <Sparkles size={11} />Upgrade to Pro — Unlock AI Insight
        </Link>
      </div>
    </div>
  );
          ...
          {setup.pairs.map((p) => (
            <div key={p.pair} className="rounded-xl border border-zinc-800/70 bg-zinc-900/40 px-3 py-3">
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className={`rounded-md border px-1.5 py-0.5 text-[8.5px] font-bold uppercase ${p.direction === "long" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-400" : "border-rose-500/30 bg-rose-500/10 text-rose-400"}`}>
                  {p.direction === "long" ? <TrendingUp className="mr-0.5 inline" size={8} /> : <TrendingDown className="mr-0.5 inline" size={8} />}
...
function SmartInsightTab({ event, hasProAccess }: { event: EconomicEvent; hasProAccess: boolean }) {
  return (
    <div>
      <div className="mb-3 flex items-center gap-2">
...
function DeepDataPanel({ event, hasProAccess }: { event: EconomicEvent; hasProAccess: boolean }) {
  ...
  return (
    <div className="border-t border-zinc-800/60 bg-zinc-950/60 px-4 pb-5 pt-3 sm:px-5">
      ...
      {activeTab === "history"     && <HistoryTab     event={event} hasProAccess={hasProAccess} />}
      {activeTab === "insight"     && <SmartInsightTab event={event} hasProAccess={hasProAccess} />}
    </div>
  );
}
...
function EventCard({ event, isLast, hasProAccess }: { event: EconomicEvent; isLast: boolean; hasProAccess: boolean }) {
  ...
  return (
    <div className={!isLast ? "border-b border-zinc-800/60" : ""}>
      ...
      >
        <div className="flex w-[68px] shrink-0 flex-col items-start gap-1 sm:w-[76px]">
          <span className="flex items-center gap-1 font-mono text-[10.5px] font-medium tabular-nums text-zinc-400">
            <Clock size={8} strokeWidth={1.75} className="text-zinc-700" />{event.time}
          </span>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5">
            {event.affectedAssets.slice(0, 3).map((a) => (
              <span key={a} className="rounded border border-zinc-800/70 bg-zinc-900/40 px-1.5 py-px text-[8.5px] font-medium text-zinc-600">{a}</span>
            ))}
            {event.affectedAssets.length > 3 && <span className="text-[8px] text-zinc-700">+{event.affectedAssets.length - 3}</span>}
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${open ? "max-h-[1400px] opacity-100" : "max-h-0 opacity-0"}`}>
        {open && <DeepDataPanel event={event} hasProAccess={hasProAccess} />}
      </div>
    </div>
  );
}

function EventFeed({ events, hasProAccess }: { events: EconomicEvent[]; hasProAccess: boolean }) {
  if (events.length === 0) return null;
  return (
    <>
      <div className="-mx-4 sm:hidden">
        {events.map((ev, i) => <EventCard key={ev.id} event={ev} isLast={i === events.length - 1} hasProAccess={hasProAccess} />)}
      </div>
...
export default function EconomicCalendarPage({ hasProAccess }: EconomicCalendarPageProps) {
      ...
      <div className="relative">
        <header className="mb-4 sm:mb-5">
          <div className="mb-1.5 flex items-center justify-between gap-2">
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-600">Smart Economic Calendar</p>
        ...
        {visible.length === 0
          ...
          : <EventFeed events={visible} hasProAccess={hasProAccess} />
        }

        <div className="mt-5 flex items-center justify-center gap-1.5">
          <Globe size={10} className="text-zinc-700" strokeWidth={1.75} />
          <p className="text-[10px] text-zinc-700">
            All times in <span className="font-medium text-zinc-600">UTC</span>. Data is indicative only. Not financial advice.
          </p>
        </div>
      </div>
    </div>
