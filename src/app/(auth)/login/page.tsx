...
//
// Trade Insight Daily — Login Page
// ─────────────────────────────────────────────────────────────────────────────
// Precision Minimalism glassmorphism card.
// Phase 1: UI only. Form state is managed locally with React state.
//           Server Actions + NextAuth signIn() wired in Phase 2.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Google Icon (SVG — lucide doesn't ship a Google logo) ───────────────────
...
function InputField({
  ...
  icon, autoComplete, suffix, disabled,
}: InputFieldProps) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        ...
        />
        {/* Right suffix (e.g. show/hide password toggle) */}
        {suffix && (
...
export default function LoginPage() {
  ...
  // ── Handlers (Phase 2 will replace these stubs with real server actions) ──
  ...
  async function handleCredentialsSubmit(e: React.FormEvent) {
    ...
    setIsLoading(true);
    try {
      // TODO Phase 2: await signIn('credentials', { email, password, redirectTo: '/dashboard' })
      ...
      >
        ...
        <div className="mb-7 text-center">
          ...
          <p className="text-[13px] font-light text-zinc-500">
            Sign in to your{' '}
            <span className="font-medium text-zinc-300">Trade Insight Daily</span> account
          </p>
        </div>
        ...
        <form onSubmit={handleCredentialsSubmit} className="flex flex-col gap-4" noValidate>
          ...
          <div className="-mt-1 flex justify-end">
            <Link
              ...
              className="text-[11px] font-medium text-zinc-600 transition-colors hover:text-sky-400"
            >
              Forgot password?
            </Link>
          </div>
          ...
          <button
            ...
            style={{ boxShadow: busy ? 'none' : '0 0 0 0 transparent' }}
            onMouseEnter={(e) => {
        ...
        <div
          className="mt-4 flex items-center justify-center gap-1.5 rounded-lg
